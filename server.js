...
const data = {
  votes: [],          // { candidate, thinkTime, timestamp, id }
  round2Votes: [],    // { candidate, thinkTime, timestamp, id }
  connections: new Set(),
  startTime: Date.now(),
  // Per-candidate think times for detailed analysis
  thinkTimes: {
    lars: [],
    youssef: [],
    sem: [],
...
// QR code endpoint - generates QR for the student site
...
// API endpoint for current stats (fallback)
...
function getStats() {
  ...
  // Per-candidate average think times
...
io.on('connection', (socket) => {
  ...
  socket.on('vote:round1', (data_vote) => {
    ...
    // Store per-candidate think time
